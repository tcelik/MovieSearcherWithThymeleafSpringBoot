...
public class HomeRestController {
    ...
    */
    ...
    {
        //sender send api key, get from request body
        System.out.println(api_key);

        //client send +, server convert plus to space, and my job is space to plus to send valid request
        query = StringUtil.changeWhiteSpaceWithPlusSignForConfiguration(query);

        //fmtUrl to url for parameter
        String fmtUrl = "https://api.themoviedb.org/3/search/movie?api_key=%s&query=%s";
        String url = String.format(fmtUrl, api_key, query);

        //Send request and get response from api with Unirest library. Unirest simplify string to json | stream to Json Part.
        try {
            //Direct Response from Unirest
            JSONArray results = (JSONArray) Unirest.get(url).asJson().getBody().getObject().get("results");

            //Result building.
            StringBuilder resultSb = new StringBuilder();

            //JSONArray for each JSONObject
            for(int i = 0; i < results.length(); ++i) {
                JSONObject jsonObject = (JSONObject) results.get(i);
                resultSb.append(jsonObject.get("poster_path") + "\n");
            }
            ...
            e.printStackTrace();
            return "Exception is about --> "+ e.getMessage();
        }

    }

    @PostMapping("3/search/moviet")
    public String searchMoviet(@RequestParam(name = "query") String query)
    {
        String api_key = "df6f5bc48cf129a2edbc1ad6bf4196e0";
        return searchMovie(api_key, query);
    }
